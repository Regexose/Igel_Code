class Zitat {
  ...
  void makePositions() {
    ...
    this.position = PVector.mult(this.firstPos, scaleVal);
  }
  ...
  void display() {
    ...
    if (this.firstPos.x < this.parentW/2) {
      ...
      layer2.translate(this.position.x, this.position.y - (this.img.height * this.scale));
    }
  ...
  void textDisplay(float x, float y) {
    layer2.beginDraw();
    // layer2.clear();
    layer2.pushMatrix();
